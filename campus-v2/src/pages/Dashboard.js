...
export function Dashboard(container) {
  ...
  const phasesHtml = state.levels && state.levels.length > 0
    ? state.levels.map((phase) => renderPhaseCard(phase, state.user)).join('')
    : '<p style="color: var(--text-secondary)">Aun no hay fases cargadas.</p>';
      ...
      <div style="flex:1;overflow-y:auto;padding:32px;position:relative;z-index:1;">
        ...
        <div class="dashboard-grid">
            ...
            <div style="margin-top:16px;">
              <a href="#/aula/${nextPhase.id}" style="text-decoration:none;">
                <button class="btn-primary" style="font-size:0.9rem;padding:10px 20px;display:inline-flex;align-items:center;gap:8px;">
                  Retomar: Fase ${nextPhase.orden} <i data-lucide="play" style="width:16px;fill:white;"></i>
                </button>
              </a>
            </div>
          </div>
          ...
          <div class="glass-panel card-progress" style="animation: fadeIn 0.6s ease-out;">
            <h3 style="margin-bottom:16px;font-weight:500;font-size:1rem;">${safeDisplayName} - Fase ${state.user?.current_level || 0}</h3>
        ...
        <div class="dashboard-grid" style="margin-top:24px; animation: fadeIn 0.8s ease-out;">
          <div class="glass-panel" style="grid-column: span 6; padding: 20px; display: flex; flex-direction: column; gap: 15px;">
            <div style="display: flex; justify-content: space-between; align-items: center;">
              <h4 style="font-size: 0.95rem; display: flex; align-items: center; gap: 8px;">
                <i data-lucide="trophy" style="width: 16px; color: var(--accent-color);"></i> Ranking del campus
              </h4>
            ...
            <div id="ranking-snippet" style="flex: 1; display: flex; flex-direction: column; gap: 10px;">
               <div style="height: 40px; background: rgba(18,37,98,0.06); border-radius: 8px; animation: pulse 2s infinite;"></div>
               <div style="height: 40px; background: rgba(18,37,98,0.06); border-radius: 8px; animation: pulse 2s infinite;"></div>
            </div>
          </div>
          ...
          <div class="glass-panel" style="grid-column: span 6; padding: 20px; display: flex; flex-direction: column; gap: 15px;">
             <div style="display: flex; justify-content: space-between; align-items: center;">
              <h4 style="font-size: 0.95rem; display: flex; align-items: center; gap: 8px;">
                <i data-lucide="calendar" style="width: 16px; color: var(--accent-color);"></i> Proxima sesion
              </h4>
            ...
            <div id="event-snippet" style="flex: 1;">
               <div style="height: 100px; background: rgba(18,37,98,0.06); border-radius: 12px; animation: pulse 2s infinite;"></div>
            </div>
          </div>
        </div>
        ...
        <div style="margin-top:40px; animation: fadeIn 1s ease-out;">
          <div style="margin-bottom:24px;display:flex;justify-content:space-between;align-items:center;">
            <h2 style="font-size:1.6rem;letter-spacing:-0.5px;">Ruta del programa</h2>
            <a href="#/ruta" style="color:var(--navy);text-decoration:none;font-size:0.9rem;font-weight:600;display:flex;align-items:center;gap:8px;background:rgba(255,215,0,0.10);padding:8px 16px;border-radius:12px;border:1px solid rgba(255,215,0,0.24);transition:0.3s;" onmouseover="this.style.background='rgba(255,215,0,0.18)'" onmouseout="this.style.background='rgba(255,215,0,0.10)'">
...
async function loadRankingSnippet() {
  ...
  try {
    ...
    container.innerHTML = profiles.map((profile, index) => `
      <div style="display:flex; justify-content:space-between; align-items:center; background:rgba(18,37,98,0.02); padding:8px 12px; border-radius:10px; border:1px solid rgba(18,37,98,0.08); transition:0.3s;" onmouseover="this.style.borderColor='rgba(255,215,0,0.24)'" onmouseout="this.style.borderColor='rgba(18,37,98,0.08)'">
        <div style="display:flex; align-items:center; gap:10px;">
          <span style="font-size:0.8rem; font-weight:700; color:${index === 0 ? 'var(--accent-color)' : 'var(--text-muted)'}">${index + 1}</span>
          <span style="font-size:0.85rem; font-weight:500;">${escapeHtml(profile.display_name || 'Estudiante')}</span>
        </div>
        <div style="display:flex; flex-direction:column; align-items:flex-end;">
          <span style="font-size:0.75rem; color:var(--accent-color); font-weight:600;">${toDisplayNumber(profile.total_xp).toLocaleString()} XP</span>
          <span style="font-size:0.65rem; color:var(--text-muted);">Fase ${toDisplayNumber(profile.current_level)}</span>
        </div>
      </div>
    `).join('');
  } catch (error) {
...
async function loadEventSnippet() {
  ...
  try {
    ...
    if (!events || events.length === 0) {
      container.innerHTML = `
        <div style="background:rgba(18,37,98,0.02); padding:15px; border-radius:12px; border:1px dashed rgba(18,37,98,0.12); text-align:center; height:100%; display:flex; align-items:center; justify-content:center;">
          <p style="font-size:0.8rem; color:var(--text-muted);">Pronto se publicaran nuevas sesiones.</p>
        </div>
      `;
      return;
    }
    ...
    container.innerHTML = `
      <div class="glass-panel" style="background:linear-gradient(135deg, rgba(255,215,0,0.10), rgba(19,125,197,0.03)); padding:16px; border-radius:12px; border:1px solid rgba(255,215,0,0.18); height:100%;">
        <div style="display:flex; gap:12px; align-items:center; margin-bottom:10px;">
          <div style="background:rgba(255,215,0,0.14); padding:5px 8px; border-radius:8px; text-align:center; border:1px solid rgba(255,215,0,0.22);">
            <div style="font-size:0.9rem; font-weight:700; color:var(--navy); line-height:1;">${day}</div>
            <div style="font-size:0.6rem; font-weight:700; color:var(--gold-dark);">${month}</div>
          </div>
          <span style="font-size:0.75rem; color:var(--text-secondary); font-weight:600;">Sesion en vivo</span>
        </div>
        <h5 style="font-size:0.9rem; margin-bottom:12px; line-height:1.4; color:var(--text-primary); font-weight:600;">${escapeHtml(event.titulo)}</h5>
